...
function ArgumentListEvaluation(realm, strictCode, env, argNodes) {
  if (Array.isArray(argNodes)) {
    ...
    try {
      for (var _iterator = argNodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        if (node_.type === "SpreadElement") {
          ...
          var spreadObj = (0, _index2.GetValue)(realm, spreadRef);
          ...
          var expr = (0, _index2.GetValue)(realm, ref);
          args.push(expr);
        }
      }
    } catch (err) {
    ...
    if (_node.expressions.length === 0) {
      ...
      var firstSub = (0, _index2.GetValue)(realm, firstSubRef);
      ...
      var restSub = _node.expressions.slice(1, _node.expressions.length).map(function (expr) {
        return (0, _index2.GetValue)(realm, env.evaluate(expr, strictCode));
      });
...
function PrepareForOrdinaryCall(realm, F, newTarget) {
  ...
  var localEnv = (0, _index2.NewFunctionEnvironment)(realm, F, newTarget);
...
function OrdinaryCallEvaluateBody(realm, F, argumentsList) {
  if (F instanceof _index.NativeFunctionValue) {
    ...
    if (F.$FunctionKind === "generator") {
      ...
      (0, _index2.FunctionDeclarationInstantiation)(realm, F, argumentsList);
      ...
      var c = context.lexicalEnvironment.evaluateAbstractCompletion(_code, F.$Strict);
      ...
      if (c instanceof _completions.JoinedAbruptCompletions) {
        ...
        return c;
      } else if (c instanceof _completions.PossiblyNormalCompletion) {
        ...
        var joinedEffects = (0, _index2.joinEffectsAndRemoveNestedReturnCompletions)(realm, c, e);
        ...
        if (result instanceof _completions.JoinedAbruptCompletions) {
          ...
          // incorporates them.
          var possiblyNormalCompletion = void 0;

          var _unbundleReturnComple = (0, _index2.unbundleReturnCompletion)(realm, result);

          var _unbundleReturnComple2 = _slicedToArray(_unbundleReturnComple, 2);

          joinedEffects = _unbundleReturnComple2[0];
          possiblyNormalCompletion = _unbundleReturnComple2[1];

          if (context.savedCompletion !== undefined) context.savedCompletion = (0, _index2.composePossiblyNormalCompletions)(realm, context.savedCompletion, possiblyNormalCompletion);else context.savedCompletion = possiblyNormalCompletion;
...
function EvaluateDirectCallWithArgList(realm, strictCode, env, ref, func, thisValue, argList, tailPosition) {
  if (func instanceof _index.AbstractValue && _Value2.default.isTypeCompatibleWith(func.getType(), _index.FunctionValue)) {
    var fullArgs = [func].concat(argList);
    return realm.deriveAbstract(_index3.TypesDomain.topVal, _index3.ValuesDomain.topVal, fullArgs, function (nodes) {
