...
var Realm = exports.Realm = function () {
  ...
  _createClass(Realm, [{
    ...
    value: function popContext(context) {
      ...
      var savedEffects = context.savedEffects;
      if (savedEffects !== undefined && this.contextStack.length > 0) {
        ...
        var ctx = this.getRunningContext();
    ...
    value: function evaluateNodeForEffects(ast, strictCode, env, state) {
      return this.evaluateForEffects(function () {
        return env.evaluateAbstractCompletion(ast, strictCode);
      }, state);
    }
  }, {
    key: "evaluateNodeForEffectsInGlobalEnv",
    value: function evaluateNodeForEffectsInGlobalEnv(node, state) {
      var _this2 = this;

      return this.wrapInGlobalEnv(function () {
        return _this2.evaluateNodeForEffects(node, false, _this2.$GlobalEnv, state);
      });
    }
  }, {
    ...
    value: function evaluateForEffects(f, state) {
      // Save old state and set up empty state for ast
      var context = this.getRunningContext();
      var savedContextEffects = context.savedEffects;
    ...
    key: "addPriorEffects",
    value: function addPriorEffects(priorEffects, subsequentEffects) {
    ...
    value: function getCapturedEffects(v) {
      var context = this.getRunningContext();
    ...
    value: function stopEffectCaptureAndUndoEffects() {
      // Roll back the state changes
      this.restoreBindings(this.modifiedBindings);
      this.restoreProperties(this.modifiedProperties);
    ...
    value: function createAbstract(types, values, args, buildNode, kind, intrinsicName) {
      (0, _invariant2.default)(this.useAbstractInterpretation);
      ...
      return new Constructor(this, types, values, args, buildNode, { kind: kind, intrinsicName: intrinsicName });
    }
  }, {
    ...
    key: "deriveAbstract",
    value: function deriveAbstract(types, values, args, buildNode, optionalArgs) {
      (0, _invariant2.default)(this.useAbstractInterpretation);
      var generator = this.generator;
      (0, _invariant2.default)(generator);
      if (types.getType() === _index.UndefinedValue) {
    ...
    value: function appendGenerator(generator) {
      ...
      if (realmGenerator === undefined) {
        (0, _invariant2.default)(generatorBody.length === 0);
        return;
      }
    ...
    value: function handleError(diagnostic) {
      if (!diagnostic.callStack && this.contextStack.length > 0) {
        var error = (0, _index3.Construct)(this, this.intrinsics.Error);
