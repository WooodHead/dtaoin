...
exports.default = function (realm) {
  ...
  // ECMA262 24.3.2
  obj.defineNativeMethod("stringify", 3, function (context, _ref5) {
    ...
    if (value instanceof _index.AbstractValue) {
      ...
      var result = realm.deriveAbstract(new _index3.TypesDomain(_index.StringValue), _index3.ValuesDomain.topVal, [value, clonedValue], function (_ref7) {
        var _ref8 = _slicedToArray(_ref7, 1),
            node = _ref8[0];

        return buildJSONStringify(realm.preludeGenerator)({
          OBJECT: node
        });
      }, { kind: "JSON.stringify(...)" });
  ...
  // ECMA262 24.3.1
  obj.defineNativeMethod("parse", 2, function (context, _ref9) {
    ...
    if (text instanceof _index.AbstractValue && text.kind === "JSON.stringify(...)") {
      ...
      (0, _invariant2.default)(args); // since text.kind === "JSON.stringify(...)" we know this must be true
      var value = args[0];
      (0, _invariant2.default)(value instanceof _index.AbstractValue); // put there by stringify above
      var clonedValue = args[1];
      var type = value.getType();
      var template = void 0;
      if (clonedValue instanceof _index.AbstractObjectValue) {
        template = InternalGetTemplate(realm, clonedValue);
        template._isSimple = realm.intrinsics.true;
      }
      var buildNode = function buildNode(_ref11) {
        var _ref12 = _slicedToArray(_ref11, 1),
            node = _ref12[0];

        return buildJSONParse(realm.preludeGenerator)({
          STRING: node
        });
      };
...
var buildJSONStringify = (0, _builder2.default)("global.JSON.stringify(OBJECT)");
var buildJSONParse = (0, _builder2.default)("global.JSON.parse(STRING)");
...
function InternalGetTemplate(realm, val) {
  var template = (0, _index2.ObjectCreate)(realm, realm.intrinsics.ObjectPrototype);
  var valTemplate = val.getTemplate();
  var _iteratorNormalCompletion2 = true;
  var _didIteratorError2 = false;
  var _iteratorError2 = undefined;

  try {
    for (var _iterator2 = valTemplate.properties[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      ...
      (0, _index2.ThrowIfMightHaveBeenDeleted)(value);
      if (value === undefined) {
        _index.AbstractValue.reportIntrospectionError(val, key); // cannot handle accessors
        throw new _errors.FatalError();
      }
      (0, _index2.CreateDataProperty)(realm, template, key, InternalJSONClone(realm, value));
    }
  } catch (err) {
...
function InternalJSONClone(realm, val) {
  if (val instanceof _index.AbstractValue) {
    if (val instanceof _index.AbstractObjectValue) {
      return realm.createAbstract(new _index3.TypesDomain(_index.ObjectValue), new _index3.ValuesDomain(new Set([InternalGetTemplate(realm, val)])), [val], function (_ref3) {
