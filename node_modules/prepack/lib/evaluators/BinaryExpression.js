  ...
  if (op === "+") {
    ...
    if (ltype === undefined || rtype === undefined) {
      ...
      if (realm.handleError(error) === "Recover") {
        ...
        if (ltype === _index.NumberValue && rtype === _index.NumberValue) return _index.NumberValue;
        return _index.Value;
      }
      throw new _errors.FatalError();
    }
    ...
    return reportErrorIfNotPure(_index2.IsToPrimitivePure, _index.BooleanValue);
  } else if (op === "===" || op === "!==") {
    return _index.BooleanValue;
  } else if (op === ">>>" || op === "<<" || op === ">>" || op === "&" || op === "|" || op === "^" || op === "**" || op === "%" || op === "/" || op === "*" || op === "-") {
    return reportErrorIfNotPure(_index2.IsToNumberPure, _index.NumberValue);
    ...
    if (rval instanceof _index.ObjectValue || rval instanceof _index.AbstractObjectValue) {
      // Simple object won't throw here, aren't proxy objects or typed arrays and do not have @@hasInstance properties.
      if (rval.isSimple()) return _index.BooleanValue;
    }
...
function computeBinary(realm, op, lval, rval, lloc, rloc) {
  // partial evaluation shortcut for a particular pattern
  if (op === "==" || op === "===" || op === "!=" || op === "!==") {
    if (!lval.mightNotBeObject() && (0, _index2.HasSomeCompatibleType)(rval, _index.NullValue, _index.UndefinedValue) || (0, _index2.HasSomeCompatibleType)(lval, _index.NullValue, _index.UndefinedValue) && !rval.mightNotBeObject()) return new _index.BooleanValue(realm, op[0] !== "=");
  }

  if (lval instanceof _index.AbstractValue || rval instanceof _index.AbstractValue) {
    var type = getPureBinaryOperationResultType(realm, op, lval, rval, lloc, rloc);
  ...
  (0, _invariant2.default)(lval instanceof _index.ConcreteValue);
  (0, _invariant2.default)(rval instanceof _index.ConcreteValue);
