...
import BaseCustomerInfo from '../../../components/boards/BaseCustomerInfo';
...
export default class Detail extends React.Component {
  ...
  componentDidMount() {
    api.ajax({url: api.customer.detail(this.state.customer_id)}, function (data) {
      this.setState({customer_detail: data.res.customer_info});
    }.bind(this));

    api.ajax({url: api.auto.detail(this.state.customer_id, this.state.auto_id)}, function (data) {
    ...
    return (
      <div>
        ...
        <div className="margin-top-40">
          ...
          <BaseCustomerInfo auto={this.state.autos} customer_id={this.state.customer_id} auto_id={this.state.auto_id}/>
