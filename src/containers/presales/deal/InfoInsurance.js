...
import {message, Form, Input, Button, Select, DatePicker, Row, Col, Switch} from 'antd';
...
class InfoInsurance extends Component {
  ...
  render() {
    ...
    let {autoDealId, autoId, customerId} = this.props;
    ...
    return (
      <div>
        <Form className={inputShow}>
          ...
          <Row>
            <Col span={6}>
              <FormItem label="被保人" {...formItemLayout} required>
            ...
            <Col span={6}>
              <NumberInput
                label="车船税"
                defaultValue={(insuranceInfo.usage_tax && Number(insuranceInfo.usage_tax).toFixed(2)) || ''}
                id="usage_tax"
                self={this}
          ...
          <Row>
            <Col span={6}>
              <NumberInput
                label="交强险"
                defaultValue={(insuranceInfo.traffic_insurance && Number(insuranceInfo.traffic_insurance).toFixed(2)) || ''}
            ...
            <Col span={6}>
              <NumberInput
                label="商业险总额"
                defaultValue={(insuranceInfo.ci_total && Number(insuranceInfo.ci_total).toFixed(2)) || ''}
                id="ci_total"
                self={this}
                layout={formItemLayout}
          ...
          <Row>
            ...
            <Col span={18}>
              <Col span={2}>
                <span style={{position: 'absolute', right: '12px', top: '5px', zIndex: '9998'}}>
          ...
          <Row>
            ...
            <Col span={6}>
              <FormItem label="押金给保险公司" {...formItemLayout}>
                {getFieldDecorator('is_deposit_ic', {
                  valuePropName: 'checked',
                  initialValue: Number(insuranceInfo.is_deposit_ic) === 1,
                })(
                  <Switch checkedChildren={'是'} unCheckedChildren={'否'} disabled={depositDisabled}/>
                )}
              </FormItem>
            </Col>
            <Col span={6}>
              <FormItem label="投保日期" {...formItemLayout}>
                {getFieldDecorator('start_date', {
                  initialValue: formatter.getMomentDate(insuranceInfo.start_date) || formatter.getMomentDate(startDate),
                })(
                  <DatePicker onChange={this.handleInsuranceDate} placeholder="起始日期" allowClear={false}/>
                )}
              </FormItem>
            </Col>
            <Col span={6}>
              <FormItem label="到期日期" {...formItemLayout}>
                {getFieldDecorator('end_date', {
                  initialValue: formatter.getMomentDate(insuranceInfo.end_date) || formatter.getMomentDate(endDate),
                })(
        ...
        <Form className={show}>
          <Row>
            ...
            <Col span={6}>
              <FormItem label="交强险单号" {...formItemLayout} required>
                <span>{insuranceInfo.insurance_num}</span>
              </FormItem>
            </Col>
            <Col span={6}>
              <FormItem label="车船税" {...formItemLayout} required>
                <span>{(insuranceInfo.usage_tax && Number(insuranceInfo.usage_tax).toFixed(2)) || ''}</span>
              </FormItem>
            </Col>
          </Row>
          ...
          <Row>
            <Col span={6}>
              <FormItem label="交强险" {...formItemLayout} required>
                <span>{(insuranceInfo.traffic_insurance && Number(insuranceInfo.traffic_insurance).toFixed(2)) || ''}</span>
              </FormItem>
            </Col>
            <Col span={6}>
              <FormItem label="商业保险公司" {...formItemLayout}>
                <span>{insuranceInfo.ci_insurance_company}</span>
              </FormItem>
            </Col>
            <Col span={6}>
              <FormItem label="商业险单号" {...formItemLayout}>
                <span>{insuranceInfo.ci_insurance_num}</span>
              </FormItem>
            </Col>
            <Col span={6}>
              <FormItem label="商业险总额" {...formItemLayout}>
                <span>{(insuranceInfo.ci_total && Number(insuranceInfo.ci_total).toFixed(2)) || ''}</span>
              </FormItem>
            </Col>
          </Row>
          <Row>
          ...
          <Row>
            <Col span={6}>
              <FormItem label="保险押金" {...formItemLayout}>
                <span>{(insuranceInfo.deposit && Number(insuranceInfo.deposit).toFixed(2)) || ''}</span>
              </FormItem>
            </Col>
            <Col span={6}>
              <FormItem label="押金给保险公司" {...formItemLayout}>
