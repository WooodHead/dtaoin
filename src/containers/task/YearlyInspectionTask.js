...
export default class YearlyInspectionTask extends Component {
  constructor(props) {
    super(props);
    this.state = {
      totalPagination: 0,
  ...
  getList(between, status, page = 1) {
    ...
    api.ajax({
      url: api.task.getYearlyInspectionTaskList(between, status, page),
    }, (data) => {
      if (data.code !== 0) {
  ...
  render() {
    ...
    const columns = [{
      ...
      className: 'center',
      render(value, record) {
        return dueExpire(record.inspection_expire_date) + '日';
      },
    }, {
      title: '年检到期',
      ...
      className: 'center',
      render(value, record) {
        return <TaskModal record={record} task_type="inspection" handleChangeData={handleChangeData}/>;
      },
    }];
    return (
      <div>
        <Row className="mb15">
          <Col span={6}>
            ...
            <Select size="large" defaultValue={String(this.state.status)} onSelect={this.handleChangeStatus} style={{width: 200}}>
          ...
          <Col span={9}>
            ...
            <Select size="large" defaultValue={String(this.state.status)} onSelect={this.handleChangeDueData} style={{width: 200}}>
