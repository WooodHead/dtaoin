...
export default class StarterCustomer extends BaseModal {
  ...
  async getIsAuthorization() {
    let hasPermission = await api.checkPermission(path.customer.reminder);
    this.setState({hasPermission});
  }

  render() {
    let record = this.props.record;
    let {data, hasPermission} = this.state;
    let contentPending = (
      ...
      <div>
        ...
        >
          <Row className="mb20">
            ...
            <Col span={12}>
              <label span={3} className="mr20">下次提醒:</label>
              <DatePicker
