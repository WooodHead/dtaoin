...
export default class ConfirmProjectFrom extends PrintThisComponent {
  ...
  handlePrint(e) {
    ...
    let qrCode = ReactDOM.findDOMNode(this.refs.qrCode),
      printImg = ReactDOM.findDOMNode(this.refs.printImg),
      printInfo = ReactDOM.findDOMNode(this.refs.printProjectOrder);
  ...
  getProjectDetail(customerId, projectId) {
    api.ajax({url: api.maintProjectByProjectId(customerId, projectId)}, function (data) {
      ...
      this.getUserAutoDetail(detail.customer_id, detail.user_auto_id);
    }.bind(this));
  }
  ...
  getUserAutoDetail(customerId, userAutoId) {
    api.ajax({url: api.getAutoDetail(customerId, userAutoId)}, function (data) {
      this.setState({auto: data.res.detail})
    }.bind(this));
  }

  render() {
    const {project, customer, auto, items, parts, payUrl, materialFee, timeFee} = this.state;
