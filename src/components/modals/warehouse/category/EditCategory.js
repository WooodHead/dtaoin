...
class EditCategory extends BaseModal {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  handleSubmit() {
    ...
    api.ajax({
      url: api.warehouse.editCategory(),
      type: 'POST',
      data: formData
    }, data => {
      this.hideModal();
        ...
        <Row key={k}>
          <Col span="12">
            <FormItem label={`配件档次${k + 1}`} labelCol={{span: 12}} wrapperCol={{span: 12}}>
              <Input {...getFieldProps(`name_${k}`, {initialValue: (k >= category.levels.length) ? '' : category.levels[k].name})}/>
            </FormItem>
          </Col>
          <Col span="7">
            <FormItem label="报价" labelCol={{span: 8}} wrapperCol={{span: 14}}>
              <Input
                type="number"
                {...getFieldProps(`price_${k}`, {initialValue: (k >= category.levels.length) ? '' : category.levels[k].price})}
                addonAfter="元"
                min={0}
                placeholder="请输入报价"
              />
    ...
    return (
      <span>
        ...
        <Modal
          ...
          <Form horizontal>
            <Input type="hidden" {...getFieldProps('_id', {initialValue: category._id})}/>
...
EditCategory = Form.create()(EditCategory);
