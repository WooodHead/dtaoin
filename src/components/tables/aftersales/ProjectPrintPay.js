...
export default class ProjectPrintPay extends PrintThisComponent {
  ...
  handlePrint(e) {
    ...
    this.printThis({
      ...
      pageTitle: App.session.brand_name+"-"+App.session.company_name+"-客户结算单",               //add title to print page
      removeInline: false,         //remove all inline styles from print elements
      printDelay: 333,            // variable print delay
      header: '<h2 style="text-align:center">'+App.session.brand_name+'-'+App.session.company_name+'-客户结算单</h2>',               // prefix to body
  ...
  render() {
    const {project, customer, auto, payUrl} = this.state;
    let items = [];                                 
    ...
    return (
      <div>
        <h3 className="center">{App.session.brand_name}-{App.session.company_name}-客户结算单</h3>
        <Form horizontal ref="printProjectOrder" className="mt15">
          <div className="border-ccc">
            <Row className="padding-tb-15 padding-l-10">
              ...
              <Col span="5">接待顾问：{App.session.name}</Col>
            ...
            <Row className="padding-tb-15 padding-l-10">
              <Col span="6" className="strong">总结算金额:{project.total_fee}元</Col>
              <Col span="10"></Col>
              <Col span="4">客户签字：</Col>
